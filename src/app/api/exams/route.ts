  ...
  try {
    ...
    const [exams, total] = await Promise.all([
      prisma.exam.findMany({
        ...
        select: {
          id: true, title: true, description: true, duration: true, questionCount: true, topicId: true, published: true,
    ...
    return apiPaginated(exams, { page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, duration, questionCount, topicId, published } = parsed.data;

    const topic = await prisma.topic.findUnique({ where: { id: topicId } });
    if (!topic) return apiError("Topic not found");

    const exam = await prisma.exam.create({
      data: { title, description, duration, questionCount, topicId, published: published ?? true },
    });
