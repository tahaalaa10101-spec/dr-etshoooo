...
export async function GET(request: NextRequest) {
  try {
    ...
    const [apps, total] = await Promise.all([
      prisma.medicalApp.findMany({ where, skip, take: limit, orderBy: { name: "asc" } }),
      prisma.medicalApp.count({ where }),
    ]);

    return apiPaginated(apps, { page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const app = await prisma.medicalApp.create({ data: parsed.data });
