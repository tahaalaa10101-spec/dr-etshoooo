  ...
  try {
    ...
    const [notes, total] = await Promise.all([
      prisma.note.findMany({
        ...
        select: {
          id: true, title: true, content: true, category: true, topicId: true, published: true,
    ...
    return apiPaginated(notes, { page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, content, category, topicId, published } = parsed.data;

    const topic = await prisma.topic.findUnique({ where: { id: topicId } });
    if (!topic) return apiError("Topic not found");

    const note = await prisma.note.create({
      data: { title, content, category, topicId, published: published ?? true },
    });
